...
class Loss(object):
    ...
    def __call__(
        ...
        **params
        ...
        return self._compute_loss(prediction_tensor, target_tensor, num,**params)
...
class WeightedL2LocalizationLoss(Loss):
    ...
    def _compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        if self._code_weights is not None:
            self._code_weights = self._code_weights.type_as(prediction_tensor).to(
    ...
    def _compute_loss(self, prediction_tensor, target_tensor, mask=None, weights=None):
        ...
        Returns:
          loss: a float tensor of shape [batch_size, num_anchors] tensor
            representing the value of the loss function.
        """
#         print("l1:",prediction_tensor.size())
        diff = prediction_tensor - target_tensor
        if mask is not None:
            diff = diff[mask]
        if self._code_weights is not None:
...
#         loss_sum = 0
#         focal_cross_entropy_loss1 = []
#         target_tensor_list = torch.chunk(target_tensor, chunks=6, dim=0)
#         prediction_tensor_list = torch.chunk(prediction_tensor, chunks=6, dim=0)
#         for i in range(6):
#     # 获取第 i 个 agent 的张量
#             target_tensor_i = target_tensor_list[i]
#             prediction_tensor_i = prediction_tensor_list[i]
#     # 进行计算
...
#                     weights *= (
#                         indices_to_dense_vector(class_indices, prediction_tensor_i.shape[2])
#                         .view(1, 1, -1)
#                         .type_as(prediction_tensor_i)
#                     )
#             per_entry_cross_ent = _softmax_cross_entropy_with_logits(
#                 labels=target_tensor_i, logits=prediction_tensor_i
#             )

#             # convert [N, num_anchors] to [N, num_anchors, num_classes]
#             per_entry_cross_ent = per_entry_cross_ent.unsqueeze(-1) * target_tensor_i
#             prediction_probabilities = F.softmax(prediction_tensor_i, dim=-1)
#             p_t = (target_tensor_i * prediction_probabilities) + (
#                 (1 - target_tensor_i) * (1 - prediction_probabilities)
#             )
# #             print(target_tensor_i.size())
# #             print(prediction_tensor_i.size())
#             modulating_factor = 1.0
#             if self._gamma:
#                 modulating_factor = torch.pow(1.0 - p_t, self._gamma)
#             alpha_weight_factor = 1.0
...
#                 modulating_factor * alpha_weight_factor * per_entry_cross_ent
#             )
# #             print(focal_cross_entropy_loss.size())
# #             focal_cross_entropy_loss1[i] =  focal_cross_entropy_loss
#             focal_cross_entropy_loss1.append(focal_cross_entropy_loss)
# #             print("loss:",focal_cross_entropy_loss1[i])
# #         for j in range(6):
# #             loss_sum = loss_sum + focal_cross_entropy_loss1[j]
        
        
#         if weights is not None:
#             return loss_sum * weights
#         else:
#             return focal_cross_entropy_loss1
    def _compute_loss(
        self, prediction_tensor, target_tensor,num, weights=None, class_indices=None
    ):
        """Compute loss function.
...
#         print(num)
        loss_sum = 0
        focal_cross_entropy_loss1 = []
        target_tensor_list = torch.chunk(target_tensor, chunks=num, dim=0)
        prediction_tensor_list = torch.chunk(prediction_tensor, chunks=num, dim=0)
        for i in range(num):
    # 获取第 i 个 agent 的张量
            target_tensor_i = target_tensor_list[i]
            prediction_tensor_i = prediction_tensor_list[i]
...
#         print(len(focal_cross_entropy_loss1))
        if weights is not None:
            return loss_sum * weights
        else:
            return focal_cross_entropy_loss1
...
class WeightedSoftmaxClassificationLoss(Loss):
    ...
    def _compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        """
        loss_cls_list = []
        weights_list = torch.chunk(weights, chunks=6, dim=0)
        prediction_tensor_list = torch.chunk(prediction_tensor, chunks=6, dim=0)
        target_tensor_list = torch.chunk(target_tensor, chunks=6, dim=0)
        for i in range(6):
            num_classes = prediction_tensor_list[i].shape[-1]
            prediction_tensor[i] = torch.div(prediction_tensor[i], self._logit_scale)
            per_row_cross_ent = _softmax_cross_entropy_with_logits(
                labels=target_tensor[i].view(-1, num_classes),
                logits=prediction_tensor[i].view(-1, num_classes),
            )
            loss_cls_list.append(per_row_cross_ent.view(weights_list[i].shape) * weights_list[i])
