...
#         print(sum(sol_vec))
    ...
    def corner_loss(self, anchors, reg_loss_mask, reg_targets, pred_result,num):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        reg_targets_list = torch.chunk(reg_targets, chunks=num, dim=0)
        pred_result_list = torch.chunk(pred_result, chunks=num, dim=0)
        anchors_list = torch.chunk(anchors, chunks=num, dim=0)
        reg_loss_mask_list = torch.chunk(reg_loss_mask, chunks=num, dim=0)
        loss_list = []
        loss_sum = 0
#         print(anchors.size())
        for i in range(num):
...
#             print("pred_corner:",pred_corners.size())
    ...
    def loss_calculator(
        ...
        loss_cls_list = []
#         print(num_agent)
        cls_list = self.criterion["cls"](result["cls"], labels,num_agent)
            ...
            if self.loss_type == "corner_loss":
                if self.only_det:
                    loss_loc,loss_loc_list= self.corner_loss(
                        anchors, reg_loss_mask, reg_targets, result["loc"],num_agent
                    )
                    loss_num += 1
    ...
    def step(self, data, batch_size,m, num_agent):
...
#         print("num:",num_agent)
        loss_collect = self.loss_calculator(
            result, anchors, reg_loss_mask, reg_targets, labels, N,num_agent
        )
...
#         loss_sum = loss_cls + loss_loc + kd_loss
        grads_list = []   
        loss2_list = []
#         print(len(loss_loc_list))
#         print(len(loss_cls_list))
        if self.MGDA:
...
#             print(len(grads_list))
            ...
            gs = self.ComputeGradient(grads_list,loss2_list,m)
...
#             print(len(param))
        ...
        if self.config.pred_type in ["motion", "center"] and not self.only_det:
            ...
            return loss_sum.item(), loss_cls.item(), loss_loc.item(), gs,m
