...
def main(config, args):
    ...
    valset1 = V2XSimSeg(
        dataset_roots=[f"/2211041005/seg_shujuji/seg_shujuji/v2x-sim3/test/agent{i}" for i in agent_idx_range],
        config=config,
        split="val",
        val=True,
        com=args.com,
    ...
    valset2 = V2XSimSeg(
        dataset_roots=[f"/2211041005/seg_shujuji/seg_shujuji/v2x-sim4/test/agent{i}" for i in agent_idx_range],
        config=config,
        split="val",
        val=True,
        com=args.com,
    ...
    valset3 = V2XSimSeg(
        dataset_roots=[f"/2211041005/seg_shujuji/seg_shujuji/v2x-sim5/test/agent{i}" for i in agent_idx_range],
        config=config,
        split="val",
        val=True,
        com=args.com,
    ...
    combined_testing_dataset = ConcatDataset([valset1,valset2,valset3]) 
    
    valloader = DataLoader(
        combined_testing_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers
    )
    ...
    for idx, sample in enumerate(tqdm(valloader)):
        ...
        pred, labels = segmodule.step(data, num_agent, batch_size, m,u,loss=False)
#         pred, labels = segmodule.step(data, num_agent, batch_size,m, loss=False)
        ...
        if args.visualization and idx % 10 == 0:  # render segmatic map
            plt.clf()
