...
from coperception.utils.CoDetModule_AR2VP import *
from coperception.utils.loss_AR2VP import *
...
def main(args):
    ...
    if args.com == "upperbound":
        ...
        if args.warp_flag:
            flag = flag + "_warp"
    elif args.com in {"v2v", "disco", "sum", "mean", "max", "cat", "agent","AR2VP"}:
        flag = args.com
...
#         rsu=args.rsu,
#     )
    testing_dataset1 = V2XSimDet(
        dataset_roots=[f"/2211041005/det_shujuji/v2x-sim3/test/agent{i}" for i in agent_idx_range],
        config=config,
        config_global=config_global,
        split="val",
        val=True,
    ...
    testing_dataset2 = V2XSimDet(
        dataset_roots=[f"/2211041005/det_shujuji/v2x-sim4/test/agent{i}" for i in agent_idx_range],
        config=config,
        config_global=config_global,
        split="val",
        val=True,
    ...
    testing_dataset3 = V2XSimDet(
        dataset_roots=[f"/2211041005/det_shujuji/v2x-sim5/test/agent{i}" for i in agent_idx_range],
        config=config,
        config_global=config_global,
        split="val",
        val=True,
    ...
    combined_testing_dataset = ConcatDataset([testing_dataset1, testing_dataset2,testing_dataset3])

    validation_data_loader = DataLoader(
        combined_testing_dataset, batch_size=1, shuffle=False, num_workers=num_workers
    )

    print("Validation dataset size:", len(combined_testing_dataset))
    ...
    fafmodule.model.load_state_dict(checkpoint["model_state_dict"],False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="lowerbound/upperbound/disco/when2com/v2v/sum/mean/max/cat/agent/AR2VP",
    )
